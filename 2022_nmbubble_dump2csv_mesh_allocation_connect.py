...
np.savetxt(f'{timestep}'+'_specs.txt', np.array([xmin, xmax, ymin, ymax, zmin, zmax, mass, density]), delimiter=",")
# 
limit = 2.4 #Angstrom - single water molecule diameter  : J. Phys. Chem. B 2010, 114, 1954–1958
#limit = 3.5 # water molecules equilibrium distance 
binsizes = np.array([limit]) # np.ceil(np.array([1, 2, 3, 4, 5, 7.5, 10]) * limit) # times limit - nominal bubble/cavity size 
...
item = 0
#
for binsize in binned[:,0]:
    left = np.where((binned[:,0] == binsize) & (binned[:,2] == binned[item,1]))
    right = np.where((binned[:,0] == binsize) & (binned[:,1] == binned[item,2]))
    front = np.where((binned[:,0] == binsize) & (binned[:,4] == binned[item,3]))
    back = np.where((binned[:,0] == binsize) & (binned[:,3] == binned[item,4]))
    top = np.where((binned[:,0] == binsize) & (binned[:,6] == binned[item,5]))
    bottom = np.where((binned[:,0] == binsize) & (binned[:,5] == binned[item,6]))
