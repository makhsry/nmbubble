...
/* ----------------------------------------------------------------------
   ...
   Editted to include hydrophobicity strength paramter - MAK 202201019
   hydro = 1.0 has no effect
...
void FixWallLJ126::precompute(int m)
{
  ...
  coeff2[m] = 24.0 * epsilon[m] * pow(sigma[m],6.0) * hydro;
  coeff3[m] = 4.0 * epsilon[m] * pow(sigma[m],12.0);
  coeff4[m] = 4.0 * epsilon[m] * pow(sigma[m],6.0) * hydro;
